...
namespace DAL
{
    public class DAL_DiemSo : DB_Connection
    {
        ...
        public DataTable LayBangDiemTBHocSinhTheoHocKy(string maLop, string maHocSinh)
        {
            ...
            string sql = "SELECT kqhk.*, hk.HESO "
                        + "FROM HOCKY hk LEFT JOIN KETQUAHOCKY kqhk ON hk.MAHOCKY = kqhk.MAHOCKY "
                         + "AND  kqhk.MaLop = '" + maLop + "' AND MaHocSinh = '" + maHocSinh + "'";
            DataTable dtBangDiem = GetTable(sql);
            ...
            foreach (string bDiemMH in listMaHKBoSung)
            {
                DataRow drbDiemBsung = dtBangDiem.NewRow();
                drbDiemBsung["MaHocSinh"] = maHocSinh;
                drbDiemBsung["MaHocKy"] = bDiemMH;
                drbDiemBsung["MaLop"] = maLop;
